...
{
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_upstream_create");
    ngx_http_upstream_t  *u;
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "u:%d", u);
    if (u == NULL) {
        return NGX_ERROR;
    }
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_upstream_create ret");
...
void
ngx_http_upstreamx_init(ngx_http_request_t *r)
{
    ngx_connection_t     *c;
...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
    ...
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_upstream_cache");
...
{
    ngx_log_debug(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_upstream_upgrade");
    int                        tcp_nodelay;
    ngx_connection_t          *c;
    ngx_http_core_loc_conf_t  *clcf;
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "finalize http upstreamx request: %i", rc);
    ...
    if (u->peer.connection) {
        ...
        if ((u->peer.connection->error == 0)
            && (!(u->peer.connection == NULL || u->peer.connection->read->eof 
                || u->peer.connection->read->error || u->peer.connection->timedout
                || u->peer.connection->write->error || u->peer.connection->write->timedout))
